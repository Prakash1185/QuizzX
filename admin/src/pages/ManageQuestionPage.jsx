      ...
      <div className="flex items-center gap-5 pb-2">
        ...
        <div>
          <Link to={`/admin/quiz/${quizId}/add-question`}>
            <button className="font-inter bg-Ngreen text-white py-2 px-5 rounded-md w-72">Add Question</button>
          </Link>
        </div>
      ...
      <div className="flex flex-col gap-5 mt-5 w-[40rem] max-h-[calc(100vh-12rem)] overflow-y-auto">
        {questions.length > 0 ? (
          questions.map((question, index) => (
            ...
            >
              <div>
                <div className="flex items-center gap-1 bg-dark2 border border-gray-500 border-opacity-20 rounded-md px-3 py-2 text-lg">
                ...
                <div className="flex flex-col gap-1.5 items-start mt-2">
                  {question.options.map((option, i) => (
                    <h1
                      key={i}
                      className="border border-gray-500 border-opacity-20 rounded-md bg-dark2 pl-4 py-2 w-full text-start overflow-x-auto"
                    >
                      {option.text}
                    </h1>
                  ))}
                ...
                <div className='flex gap-5 mt-4 w-full'>
                  <Link to={`/admin/quiz/${quizId}/edit-question/${question._id}`} className="w-1/2">
